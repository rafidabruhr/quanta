...
Part I: 
Theory
The Wave Function ($\Psi$): The Schrödinger Equation, statistical interpretation, probability currents, and conservation of normalization. The Uncertainty Principle ($\sigma_x \sigma_p \ge \hbar/2$) stemming from wave properties.
The Time-Independent Schrödinger Equation: Stationary states, separation of variables, and boundary conditions.Classic potentials: Infinite square well, finite square well, free particle (wave packets, phase/group velocity), and the $\delta$-function potential.
The Quantum Harmonic Oscillator: Solving via the algebraic method (Ladder operators $a_{\pm}$) and the analytic method (power series/Hermite polynomials).
